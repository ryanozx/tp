...
There is a status field of leave that is `PENDING` by default when a leave is added. Please DO NOT enter any status field of the leave, no recognition of status field is provided.
...
| `This leave has already existed for the employee` | The date duration entered has an overlap with the existing leave of the same employee | Double check if the dates for the new leave do not overlap with the existing leaves by using [find-leave]() or [find-leave-range](). If the leave date duration overlaps with the existing one, you may consider add the additional date duration that do not overlap as a new leave.|
